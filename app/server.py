...
def get_live_system_capabilities(prompt_token: str = "") -> str:
    ...
    return f"""⚡ **تقرير الفحص الفوري والمباشر لإمكانيات ومقدرات منظومة Sasa AI الموحدة** ⚡
**(تطوير وإشراف: الشيخ الهلباوي | {now_str_arab} - {today_str_arab})**

تم فحص البيئة التشغيلية والأنظمة الخلفية الحية الآن، وإليك الحالة المباشرة والديناميكية لجميع قدرات ومكونات المنصة المدمجة ككتلة واحدة:

1. 🟢 **نظام التحكم بمستودعات GitHub (GitHub REST API Engine)**:
   - **الحالة**: {gh_status}
   - **القدرات**: قراءة الشجرة الكلية، الفحص العميق للمستودعات، إنشاء/حذف المستودعات، والرفع المباشر مع الحصول على SHA.

2. ☁️ **محرك إدارة النشر السحابي (Render Cloud API Subsystem)**:
   - **الحالة**: {render_status}
   - **القدرات**: جلب قائمة الخدمات، تحفيز عملية النشر السحابي (Trigger Deploy)، ومراقبة حالة البناء في Render.

3. 🗄️ **قاعدة البيانات السحابية (Render PostgreSQL Database Engine)**:
   - **الحالة**: {pg_status}
   - **القدرات**: تخزين البيانات المستمرة، السجلات الفورية، وطوابير المهام.

4. 🎬 **نظام توليد الفيديوهات والوسائط (Sasa AI Video Synthesizer)**:
   - **الحالة**: {video_engine_status}
   - **القدرات**: توليد مشاهد فيديو تفاعلية، بناء بطاقات الفيديو المتحركة، وتوليد الشفرات المرئية مع إتاحة المشغلات المباشرة.

5. 🧠 **عقل الذكاء الاصطناعي ومعالجة الأكواد (Gemini LLM Subsystem)**:
   - **الحالة**: {gemini_status}
   - **القدرات**: التحليل البرمجي الفائق، تصحيح الأخطاء، وإنشاء تطبيقات أندرويد وويب متكاملة.

6. 💻 **محرك الأوامر والطرفية (Terminal & Execution Engine - `/api/execute`)**:
   - **الحالة**: {shell_status}
   - **القدرات**: تشغيل الأوامر البرمجية والسكربتات واختبار مخرجات الأوامر مباشرة.

7. 🔄 **عقل الأوركسترا الخلفي الموحد (Unified Background Orchestrator Thread)**:
   - **الحالة**: ✅ يعمل بانسجام شفاف ككتلة واحدة في الخلفية (دورة رقم `#{orch_cycles}`)
   - **القدرات**: المراقبة الذاتية 24/7، التعافي التلقائي، ومزامنة السجلات والخدمات بشكل مستمر.

8. 📜 **نظام السجلات الحية والشفافية (Live Logging System - `/api/logs`)**:
   - **الحالة**: ✅ نشط ومسجّل عدد `{logs_count}` عملية حية حتى الآن."""
