...
# ── Health, Sciences, Esoteric & Web Ingestion Engines ──
try:
    ...
    from app.esoteric.esoteric_engine import esoteric_engine, ESOTERIC_SPIRITUAL_TREATISES, UNIVERSAL_PLANES_ATLAS, SPIRITUAL_HEALERS_ENCYCLOPEDIA
    from app.tools.web_ingestion import knowledge_ingestion_tool
except ImportError:
    try:
        ...
        from esoteric.esoteric_engine import esoteric_engine, ESOTERIC_SPIRITUAL_TREATISES, UNIVERSAL_PLANES_ATLAS, SPIRITUAL_HEALERS_ENCYCLOPEDIA
        from tools.web_ingestion import knowledge_ingestion_tool
    except ImportError:
        neama_health_engine = None
        universal_science_engine = None
        esoteric_engine = None
        knowledge_ingestion_tool = None
        HERBAL_DATABASE = {}
        ANCIENT_TREATISES = {}
        MEDICAL_CURRICULUM = []
        CLINICAL_CASES = []
        UNIVERSAL_SCIENCES_CATALOG = {}
        ESOTERIC_SPIRITUAL_TREATISES = {}
        UNIVERSAL_PLANES_ATLAS = {}
        SPIRITUAL_HEALERS_ENCYCLOPEDIA = []
...
<script>
    ...
    <!-- Neama Health & Bio-Engine Modal -->
    <div id="neama-health-modal" class="saas-modal-backdrop" onclick="if(event.target===this) closeHealthModal()">
        <div class="saas-modal-card" style="max-width: 900px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <div>
                        <h2 style="font-size: 18px; font-weight: 800; color: #fff; margin: 0;">النواة الطبية الشاملة والعلوم الحيوية (Neama Health & Bio-Engine)</h2>
                        <span style="font-size: 11px; color: #34d399;">المشخص السريري، خبير العطارة والطب البديل، كتب التراث، والأكاديمية الطبية التفاعلية</span>
                    </div>
                </div>
                <button onclick="closeHealthModal()" style="background: none; border: none; color: #94a3b8; font-size: 20px; cursor: pointer;">✕</button>
            </div>

            <div style="display: flex; gap: 8px; margin-bottom: 14px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 8px;">
                <button class="tenant-chip" onclick="switchHealthTab('herbal')" id="htab-herbal" style="background: rgba(16, 185, 129, 0.2); border-color: #10b981; color: #6ee7b7;">🌿 خبير العطارة والطب البديل</button>
                <button class="tenant-chip" onclick="switchHealthTab('clinical')" id="htab-clinical">🩺 المشخص السريري</button>
                <button class="tenant-chip" onclick="switchHealthTab('treatises')" id="htab-treatises">📜 كتب طب القدماء</button>
                <button class="tenant-chip" onclick="switchHealthTab('academy')" id="htab-academy">🎓 الأكاديمية والمحاكاة</button>
                <button class="tenant-chip" onclick="switchHealthTab('anatomy')" id="htab-anatomy">🫀 أجهزة الجسم التفاعلية</button>
            </div>

            <div id="hsec-herbal" class="health-sub-sec">
                <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                    <input id="herbal-search-input" type="text" placeholder="ابحث عن عشبة (الكركم، الزنجبيل...) أو حالة صحية (القولون...)" style="flex: 1; background: rgba(255,255,255,0.06); border: 1px solid var(--border-subtle); color: #fff; padding: 8px 12px; border-radius: 6px; font-family: 'Cairo'; font-size: 12px;">
                    <button onclick="searchHerbalConsult()" style="padding: 8px 18px; background: rgba(16, 185, 129, 0.25); border: 1px solid #10b981; color: #a7f3d0; border-radius: 6px; font-weight: 700; font-size: 12px; cursor: pointer;">بحث في الأعشاب</button>
                </div>
                <div id="herbal-results-container" style="max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px;"></div>
            </div>

            <div id="hsec-clinical" class="health-sub-sec" style="display: none;">
                <div style="background: rgba(14, 22, 38, 0.7); border: 1px solid var(--border-subtle); border-radius: 10px; padding: 14px; margin-bottom: 12px;">
                    <div style="font-size: 13px; font-weight: 700; color: #fff; margin-bottom: 8px;">🩺 الاستدلال السريري المتقدم:</div>
                    <textarea id="clinical-symptoms-input" placeholder="صف الأعراض بالتفصيل..." style="width: 100%; height: 75px; background: rgba(255,255,255,0.06); border: 1px solid var(--border-subtle); color: #fff; padding: 8px; border-radius: 6px; font-family: 'Cairo'; font-size: 12px; margin-bottom: 8px;"></textarea>
                    <input id="clinical-history-input" type="text" placeholder="السجل المرضي (اختياري)..." style="width: 100%; background: rgba(255,255,255,0.06); border: 1px solid var(--border-subtle); color: #fff; padding: 8px; border-radius: 6px; font-family: 'Cairo'; font-size: 12px; margin-bottom: 8px;">
                    <button onclick="runClinicalConsult()" style="width: 100%; padding: 9px; background: rgba(56, 189, 248, 0.25); border: 1px solid #38bdf8; color: #7dd3fc; border-radius: 6px; font-weight: 700; font-size: 12.5px; cursor: pointer;">⚡ تشغيل التحليل السريري وحارس الأمان</button>
                </div>
                <div id="clinical-result-container" style="max-height: 280px; overflow-y: auto;"></div>
            </div>

            <div id="hsec-treatises" class="health-sub-sec" style="display: none;">
                <div id="ancient-treatises-container" style="max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px;"></div>
            </div>

            <div id="hsec-academy" class="health-sub-sec" style="display: none;">
                <div id="academy-curriculum-container" style="max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px;"></div>
            </div>

            <div id="hsec-anatomy" class="health-sub-sec" style="display: none;">
                <div id="anatomy-systems-container" style="max-height: 380px; overflow-y: auto; display: grid; grid-template-columns: 1fr 1fr; gap: 10px;"></div>
            </div>

            <div style="margin-top: 14px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; padding: 8px 12px; font-size: 11px; color: #fca5a5;">
                🛡️ <b>بروتوكول الأمان الطبي (Safety Guardrail):</b> هذا النظام يقدم معلومات أكاديمية وعشبية ولا يغني عن الاستشارة السريرية المباشرة. في الطوارئ اتصل بالإسعاف فوراً.
            </div>
        </div>
    </div>

    <!-- Universal Sciences Modal -->
    <div id="universal-sciences-modal" class="saas-modal-backdrop" onclick="if(event.target===this) closeSciencesModal()">
        <div class="saas-modal-card" style="max-width: 860px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span style="font-size: 26px;">🔬</span>
                    <div>
                        <h2 style="font-size: 18px; font-weight: 800; color: #fff; margin: 0;">موسوعة العلوم الكونية الشاملة (Universal Cosmic Sciences)</h2>
                        <span style="font-size: 11px; color: #c084fc;">فيزياء الكم والنسبية، الكيمياء، الفلك، الجينوم، الرياضيات البحتة، والتاريخ</span>
                    </div>
                </div>
                <button onclick="closeSciencesModal()" style="background: none; border: none; color: #94a3b8; font-size: 20px; cursor: pointer;">✕</button>
            </div>
            <div id="sciences-catalog-container" style="max-height: 440px; overflow-y: auto; display: grid; grid-template-columns: 1fr 1fr; gap: 10px;"></div>
        </div>
    </div>

    <!-- Esoteric, Spiritual Medicine & Universal Web Ingestion Modal -->
    <div id="neama-esoteric-modal" class="saas-modal-backdrop" onclick="if(event.target===this) closeEsotericModal()">
        <div class="saas-modal-card" style="max-width: 950px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 10px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span style="font-size: 26px;">📜</span>
                    <div>
                        <h2 style="font-size: 18px; font-weight: 800; color: #fff; margin: 0;">الطب الروحاني والمعرفة النورانية واستيعاب الويب (Esoteric & Universal RAG)</h2>
                        <span style="font-size: 11px; color: #f472b6;">التراث الفلسفي والروحاني، طب القلوب والنفس، أطلس العوالم، ووكيل الاستيعاب والتفريغ الذكي</span>
                    </div>
                </div>
                <button onclick="closeEsotericModal()" style="background: none; border: none; color: #94a3b8; font-size: 20px; cursor: pointer;">✕</button>
            </div>

            <!-- Sub Tabs -->
            <div style="display: flex; gap: 8px; margin-bottom: 14px; border-bottom: 1px solid rgba(255,255,255,0.08); padding-bottom: 8px; overflow-x: auto;">
                <button class="tenant-chip" onclick="switchEsotericTab('treatises')" id="etab-treatises" style="background: rgba(236, 72, 153, 0.2); border-color: #ec4899; color: #fbcfe8;">📜 كتب الطب الروحاني والتراث</button>
                <button class="tenant-chip" onclick="switchEsotericTab('planes')" id="etab-planes">🌌 أطلس العوالم العلوية والسفلية</button>
                <button class="tenant-chip" onclick="switchEsotericTab('scholars')" id="etab-scholars">🧙‍♂️ حكماء وفلاسفة الروح</button>
                <button class="tenant-chip" onclick="switchEsotericTab('ingestion')" id="etab-ingestion">🌐 وكيل الاستيعاب والأرشفة الذاتي</button>
            </div>

            <!-- Tab 1: Treatises -->
            <div id="esec-treatises" class="esoteric-sub-sec">
                <div style="display: flex; gap: 8px; margin-bottom: 12px;">
                    <input id="esoteric-search-input" type="text" placeholder="ابحث في كتب الطب الروحاني (الرازي، الغزالي، ابن سينا، السهروردي، ابن عربي...)" style="flex: 1; background: rgba(255,255,255,0.06); border: 1px solid var(--border-subtle); color: #fff; padding: 8px 12px; border-radius: 6px; font-family: 'Cairo'; font-size: 12px;">
                    <button onclick="searchEsotericTreatises()" style="padding: 8px 18px; background: rgba(236, 72, 153, 0.25); border: 1px solid #ec4899; color: #fbcfe8; border-radius: 6px; font-weight: 700; font-size: 12px; cursor: pointer;">بحث في التراث</button>
                </div>
                <div id="esoteric-treatises-container" style="max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px;"></div>
            </div>

            <!-- Tab 2: Planes Atlas -->
            <div id="esec-planes" class="esoteric-sub-sec" style="display: none;">
                <div id="esoteric-planes-container" style="max-height: 380px; overflow-y: auto; display: grid; grid-template-columns: 1fr 1fr; gap: 10px;"></div>
            </div>

            <!-- Tab 3: Scholars -->
            <div id="esec-scholars" class="esoteric-sub-sec" style="display: none;">
                <div id="esoteric-scholars-container" style="max-height: 380px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px;"></div>
            </div>

            <!-- Tab 4: Autonomous Web Ingestion -->
            <div id="esec-ingestion" class="esoteric-sub-sec" style="display: none;">
                <div style="background: rgba(14, 22, 38, 0.75); border: 1px solid rgba(6, 182, 212, 0.3); border-radius: 10px; padding: 14px; margin-bottom: 12px;">
                    <div style="font-size: 13px; font-weight: 800; color: #67e8f9; margin-bottom: 6px;">🌐 محرك البحث والاستيعاب الشامل والأرشفة (Universal Web Scraper & Vector Ingestion):</div>
                    <div style="font-size: 11px; color: #94a3b8; margin-bottom: 10px;">يقوم الوكيل بالبحث المفتوح عن أي موضوع تراثي أو علمي، قراءة الصفحات وتفريغها، تنظيف النص، تقطيعه إلى Chunks وحقنه فوراً في عقل المنظومة.</div>
                    <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                        <input id="ingestion-query-input" type="text" placeholder="اكتب استعلام البحث لاستيعابه (مثال: مخطوطات ابن سينا، تاريخ الفلسفة الإشراقية...)" style="flex: 1; background: rgba(255,255,255,0.06); border: 1px solid var(--border-subtle); color: #fff; padding: 8px 12px; border-radius: 6px; font-family: 'Cairo'; font-size: 12px;">
                        <button onclick="runWebIngestion()" style="padding: 8px 18px; background: rgba(6, 182, 212, 0.25); border: 1px solid #06b6d4; color: #a5f3fc; border-radius: 6px; font-weight: 700; font-size: 12px; cursor: pointer;">🚀 إطلاق البحث والاستيعاب</button>
                    </div>
                </div>
                <div id="ingestion-status-container" style="max-height: 280px; overflow-y: auto;"></div>
            </div>

            <!-- Safety Guardrail Banner -->
            <div style="margin-top: 14px; background: rgba(168, 85, 247, 0.1); border: 1px solid rgba(168, 85, 247, 0.3); border-radius: 8px; padding: 8px 12px; font-size: 11px; color: #e9d5ff; display: flex; align-items: center; gap: 8px;">
                <span>🛡️</span>
                <span><b>بروتوكول الأمان والمنهجية الأكاديمية:</b> هذا القطاع مخصص لدراسة التراث الفلسفي والروحاني وتاريخ الأفكار وسيكولوجيا الطب القديم، ويقدم المعلومات كإرث حضاري إنساني تحليلي.</span>
            </div>
        </div>
    </div>
...
if USE_FASTAPI:
    ...
    class BuiltInRequestHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            # ── Neama Health & Bio-Engine Endpoints ──
            elif path == "/api/health/consult/herbal":
                query = query_params.get("query", [""])[0]
                if neama_health_engine:
                    result = neama_health_engine.consult_herbal_knowledge(query)
                else:
                    result = {"success": True, "remedies": HERBAL_DATABASE}
                self._send_json_response(result)
                return

            elif path == "/api/health/consult/clinical":
                ...
                if neama_health_engine:
                    result = neama_health_engine.consult_clinical_symptoms(symptoms, medical_history)
                else:
                    result = {"success": True, "analysis": {"primary_considerations": [], "red_flags_to_watch": [], "recommended_investigations": []}}
                self._send_json_response(result)
                return

            elif path == "/api/health/treatises":
                result = {"success": True, "treatises": ANCIENT_TREATISES}
                self._send_json_response(result)
                return

            elif path == "/api/health/academy":
                result = {
                    "success": True,
            ...
            elif path == "/api/health/anatomy":
                if neama_health_engine:
                    result = neama_health_engine.get_interactive_anatomy_systems()
                else:
                    result = {"success": True, "systems": []}
                self._send_json_response(result)
                return

            # ── Universal Sciences Endpoints ──
            elif path == "/api/sciences/catalog":
                if universal_science_engine:
                    result = universal_science_engine.get_full_catalog()
                else:
                    result = {"success": True, "disciplines": UNIVERSAL_SCIENCES_CATALOG}
                self._send_json_response(result)
                return

            # ── Esoteric & Spiritual Medicine & Universal Planes Endpoints ──
            elif path == "/api/esoteric/spiritual":
                query = query_params.get("query", [""])[0]
                if esoteric_engine:
                    result = esoteric_engine.query_spiritual_medicine(query)
                else:
                    result = {"success": True, "treatises": ESOTERIC_SPIRITUAL_TREATISES, "planes": UNIVERSAL_PLANES_ATLAS, "scholars": SPIRITUAL_HEALERS_ENCYCLOPEDIA}
                self._send_json_response(result)
                return

            elif path == "/api/esoteric/planes":
                if esoteric_engine:
                    result = esoteric_engine.get_planes_atlas()
                else:
                    result = {"success": True, "planes": UNIVERSAL_PLANES_ATLAS}
                self._send_json_response(result)
                return

            # ── Universal Ingestion & Web Scraping Endpoints ──
            elif path == "/api/ingestion/search_scrape":
                content_length = int(self.headers.get("Content-Length", 0))
                post_data = self.rfile.read(content_length).decode("utf-8") if content_length > 0 else ""
                try:
                    payload = json.loads(post_data) if post_data else {}
                ...
                if knowledge_ingestion_tool:
                    scraped = knowledge_ingestion_tool.search_and_scrape(query, max_results=max_results)
                    ingest_result = knowledge_ingestion_tool.chunk_and_vectorize(scraped)
                    res = {"success": True, "scraped_docs": scraped, "ingestion_result": ingest_result}
                else:
                    res = {"success": True, "scraped_docs": [], "ingestion_result": {"status": "ok"}}
                self._send_json_response(res)
                return

            elif path == "/api/ingestion/direct":
                content_length = int(self.headers.get("Content-Length", 0))
                post_data = self.rfile.read(content_length).decode("utf-8") if content_length > 0 else ""
                try:
                    payload = json.loads(post_data) if post_data else {}
                    ...
                    payload = {}
                title = payload.get("title", "نص معرفي")
                content = payload.get("content", "")
                category = payload.get("category", "Spiritual & Historical Heritage")
                if knowledge_ingestion_tool and content:
                    ingest_result = knowledge_ingestion_tool.direct_ingest_text(title, content, category)
                    res = {"success": True, "ingestion_result": ingest_result}
                else:
